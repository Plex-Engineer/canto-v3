...
import { DELEGATE_FEE, UNDELEGATE_FEE, REDELEGATE_FEE, CLAIM_STAKING_REWARD_FEE } from "@/config/consts/fees";
  ...
  switch (txParams.txType) {
    case StakingTxTypes.DELEGATE:
       ...
       if(BigNumber(txParams.nativeBalance).isLessThan(DELEGATE_FEE.amount)){
        return { error: true, reason: TX_ERROR_TYPES.NOT_ENOUGH_NATIVE_BALANCE_STAKING };
      }
    ...
    case StakingTxTypes.CLAIM_REWARDS: {
       // check if user has enough gas balance
       if(BigNumber(txParams.nativeBalance).isLessThan(CLAIM_STAKING_REWARD_FEE.amount)){
        return { error: true, reason: TX_ERROR_TYPES.NOT_ENOUGH_NATIVE_BALANCE_STAKING };
      }
      return { error: false };
    }
