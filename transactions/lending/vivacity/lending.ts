...
import {
  ...
  getVCNoteAmountFromNote,
  maxAmountForLendingTx,
} from ".";
import { _lendingCTokenTx, _withdrawAllCTokenTx } from "./txCreators";
...
export async function cTokenLendingTx(
  ...
  try {
    ...
    const vcNoteRouterAddress = getVivacityAddress(
      txParams.chainId,
      "vcNoteRouter"
    );

    // calculate amount of vcNote from note amount and exchange rate (required for withdraw transaction)
    const vcNoteAmount = getVCNoteAmountFromNote(
      txParams.amount,
      txParams.cToken.exchangeRate,
    ...
    if (txParams.txType === CTokenLendingTxTypes.SUPPLY) {
      ...
      const { data: allowanceTxs, error: allowanceError } =
        await createApprovalTxs(
          ...
          [approvalAmount],
          {
            address: vcNoteRouterAddress as `0x${string}`,
            name: "Lending Market",
          }
        );
...
export function validateCTokenLendingTxParams(
  ...
  const maxAmount = maxAmountForLendingTx(txParams.txType, txParams.cToken);
  // validate amount
