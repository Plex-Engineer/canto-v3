...
export const GenerateUnbondingDelegationsTableRow = (userStakedValidator: UserUnbondingDelegation, index: number, onDelegate: (validator: Validator)=>void) => 
[
  <Container key={`name_${index}`}><Text font="rm_mono">{userStakedValidator?.validator_address}</Text></Container>,
  <Container key={`mystake_${index}`} direction="row" center={{horizontal: true, vertical:true}} gap="auto">
      <Text font="rm_mono">{formatBigBalance(formatBalance(userStakedValidator.entries[0].balance,18)).shortAmount + formatBigBalance(formatBalance(userStakedValidator.entries[0].balance,18)).suffix} </Text>
      <div>{" "}</div>
      <Icon
        style={{ marginLeft: "5px" }}
        icon={{
...
<Container key={`commission_${index}`}><Text font="rm_mono">{formatBalance(userStakedValidator.entries[0].balance,-2,{commify:true,precision:2})}%</Text></Container>,
<Container key={`buttonManage_${index}`}>
