...
export async function getVCNoteData(
  ...
  try {
    ...
    const [ vcNoteExchangeRateCurrent, cNoteExchangeRateCurrent] = await multicall({
      chainId,
    ...
    const vcNoteToNoteExchangeRate = (new BigNumber(vcNoteExchangeRateCurrent.result.toString())).multipliedBy(new BigNumber(cNoteExchangeRateCurrent.result.toString())).dividedBy(new BigNumber(10).pow(36))
